// module-level state used across functions
...
function fxn_is_parallel() {
  ...
  if (dmi.is_parallel.checked) {
    document.getElementById("psdiv").style.display = "block"
    var np = document.getElementById("num_procs")
...
function check_bottleneck() {
  var bgen = dmi.bottleneck_generation.value
  ...
  var e = document.getElementById("pop_growth_model")
...
function compute_memory() {
  ...
  var dyn_mem_reqd =  max_del*max_size*sizeof_int; //dmutn
  ...
  // convert to MB
  var mem_est = (dyn_mem_reqd + mem_reqd)/1024/1024
  mem_est = Math.round(mem_est*100)/100

  var msg = "Memory required for run: " + mem_est + " MB"
...
function fxn_polygenic_beneficials(init) {
  fraction_neutral = dmi.fraction_neutral.value
  var fraction_fav_mutn = dmi.frac_fav_mutn.value
  var plot_allele_gens = dmi.plot_allele_gens.value
  var tag = "Waiting time"
  if (dmi.polygenic_beneficials.checked) {
    dmi.polygenic_init.readOnly = false
    dmi.polygenic_target.readOnly = false
    dmi.polygenic_effect.readOnly = false
...
function fxn_fitness_distrib_type_init() {
  var fdt = dmi.fitness_distrib_type.value
...
function generate_mutations() {
    ...
    var nmutn = pop_size * num_linkage_subunits * 2
    if (nmutn > 10000) {
    ...
    var new_chromo_array = Array()

    for (var c in chromo_array) {
        var x = chromo_array[c].split("-")
        console.log('x:' + x)
        var start = parseInt(x[0])
    ...
    var contrast_degree = document.getElementById("contrast_degree").value

    var allele_id = 0

    for (var c in new_chromo_array) {
        var chromosome = new_chromo_array[c]
        var lb1 = (chromosome-1)*linkage_subunits_per_chromosome + 1
        var lb2 = chromosome*linkage_subunits_per_chromosome
        for (var lb = lb1; lb <= lb2; lb++) {
            var x = Math.random()
            var y = Math.random()
            var fitness;
            if (y > frac_fav_mutn) {
            ...
            for (var i = 1; i <= pop_size; i++) {
                for (var haplotype = 1; haplotype <= 2; haplotype++) {
                    allele_id += 1
                    if (contrasting) {
                        var o234 = (i-1)*pop_size + haplotype
...
function fxn_migration() {
  var x = parseInt(1*dmi.num_indiv_exchanged.value)
  var max = parseInt(1*dmi.pop_size.value)
  if (x == 0) {
...
function fxn_tribes(max_tribes) {
  var myobject = dmi.num_procs
  var num_procs = myobject.value
  ...
  for (var i = 0; i < num_procs; i++) {
    var a = (i+1)/1000 + ''; // compute number of tribe as a string
    var b = a.substring(1);  // remove the leading 0 from 0.001, 0.002, etc.
...
function fxn_selection_init() {
  var i = dmi.selection_scheme.value
  if (i == 1 || i == 2 || i == 4) {
